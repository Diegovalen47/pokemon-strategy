import type {
  MovementLocalRepository,
  MovementRemoteRepository
} from '../domain'
...
export class MovementService {
  ...
  async getAllMovementsAndSaveInDb(): Promise<void> {
    let { next, movements } =
      await this.movementRemoteRepository.getMovementsFirstList()

    while (next) {
      await this.movementLocalRepository.insertMovements(movements)
      const response =
        await this.movementRemoteRepository.getMovementsNextList(next)
      next = response.next
